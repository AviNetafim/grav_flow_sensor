...
KeepCfg kc;																															
ProgramParameters new_params(0,4,0,0);                                	// new configuration parameter - define required pramaters , timeouts
...
static void weight_test_task(void *arg){
  ...
  ADS1231::Sample sample;																										
	while (true) {
		if (!adc.poll(sample)) {
			// At 10 SPS and a 100 ms task period, sometimes you may call, slightly before DRDY goes low. This is normal.
			// add average for configurable number of samples const structure like radio (!) or  rely on pc app to average
...
// pass parameters through mutex protected data structure
// starts by reg_action, termination is signaled in data structure
// vol_stat  is cleared by arg_action before task is activated 
// data structure:
// 	run
// 	time to low sensor
// 	low sensor timeout 
...
void reg_action(regs_action& areg_act){																	
	...
	// register map
	// 0 mode[1] 1 (weight) 0 (volume), written by client, used here to select test task type no action here
	// 1 target_weight[1] in 10mg units, written by client, pass to weight_test task data structre, no action here 
	// 2 test[1] 1 to start a test, written by client, used here to activate the selected task,  then cleared 
	// 3 vol_stat[2] read by cilent, polled periodically when volume test is running, copy task data to register 
	// 4 weight_stat[3] read by cilent, polled periodically when weight test is running,copy task data to register 
	// 5 samples[50] access mutex controlled samples array and copy to register
	// 6 pointer[1]  writen by client, copy 50 samples  from weight data structure to samples starting at pointer
	// 7 actuator[2]
	// 8 actuate[1]
...
extern "C" void app_main(void){ 
  ...
  xTaskCreate(weight_test_task, "weight_test_task", 4096, nullptr, 5, nullptr); // diff between NULL and nulptr (?)
