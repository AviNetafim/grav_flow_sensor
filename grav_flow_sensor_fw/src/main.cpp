...
static void weight_test_task(void *arg){
    ...
    while (true) {
        ...
        ESP_LOGI(TAG, "Tare raw = %" PRId32 "" , tare_raw);
        const int64_t target_raw_0 = static_cast<int64_t>(-CAL_OFFSET * CAL_DIV / CAL_MUL);
        ESP_LOGI(TAG, "target 0 = %" PRId32 "" , target_raw_0);
        ...
        ESP_LOGI(TAG, "target delta = %" PRId32 "" , target_delta_raw);
        ...
        ESP_LOGI(TAG, "Weight test started: target=%" PRId32 " x 10 mg, timeout=%u ds",
